...
bool ProcessInlineHexColor(std::string_view hexColorStr, ImVec4& color) {
    ...
    if (ec != std::errc{}) {
        //std::cerr << "Error: Failed to convert hex string to integer: " << ec.message() << std::endl; 
        return false;
    }
    ...
    color.w = (hexColorStr.size() == 8) ? static_cast<float>((hexColor & 0xFF000000) >> 24) / 255.0f : 1.0f;
    return true;
}
